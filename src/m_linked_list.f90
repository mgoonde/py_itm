...
!! Edit (Miha, nov. 2023):
!! - do not allow user-defined [[key_class]] module;
!! - rename "list" -> "linked_list";
!! - rename "module linked_list_module" -> "module m_linked_list";
!! - initialize with case_sensitive=.false. by default;
...
!  Returns true if the key is present in the list
    ...
    class(linked_list),intent(inout) :: me
    class(*),intent(in)       :: key
    logical                   :: has_key
...
!  list constructor.
    ...
    logical,intent(in), optional :: case_sensitive !! if true, then string key
                                                   !! searches are case sensitive.
    !! false by default
    lst%case_sensitive = .false.
    if( present( case_sensitive) ) lst%case_sensitive = case_sensitive
...
!  just a wrapper for [[destroy_list]].
    ...
    type(linked_list),intent(inout) :: me
...
!  Returns a pointer to the data stored in the list.
    ...
    class(linked_list),intent(in)       :: me
    class(*),intent(in)          :: key
    class(*),pointer,intent(out) :: value
